...
### Breaking Changes
* The health check page now requires login by default, following NetBox's `LOGIN_REQUIRED`; NetBox API tokens are accepted, so monitoring can authenticate without a session. Set `login_required: False` in `PLUGINS_CONFIG` to keep the page open to unauthenticated probes ([#1](https://github.com/netbox-community/netbox-healthcheck-plugin/issues/1))
